...
resource "google_compute_url_map" "mcart_apigw" {
  ...
  default_service = google_compute_backend_service.mcart_apigw.id
}

resource "google_compute_target_https_proxy" "mcart_apigw" {
  name             = "mcart-apigw-https-proxy"
  url_map          = google_compute_url_map.mcart_apigw.id
  ssl_certificates = [google_compute_managed_ssl_certificate.mcart.id]
}

resource "google_compute_global_forwarding_rule" "mcart_apigw_https" {
  name                  = "mcart-apigw-https"
  target                = google_compute_target_https_proxy.mcart_apigw.id
  port_range            = "443"
  load_balancing_scheme = "EXTERNAL_MANAGED"
